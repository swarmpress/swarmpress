...
import type { ContentType, PageTemplate } from '@swarm-press/shared'
...
interface NodePaletteProps {
  ...
  onAddFromTemplate?: (templateId: string, template: PageTemplate) => void
...
export function NodePalette({
  ...
  const [selectedTemplateTag, setSelectedTemplateTag] = useState<string | undefined>()
  ...
  const handleSelectTemplate = (templateId: string, template: PageTemplate) => {
    if (onAddFromTemplate) {
      onAddFromTemplate(templateId, template)
    }
  }
  ...
  return (
    <div className="w-64 border-r bg-background flex flex-col h-full">
      ...
      <div className="p-4 border-b">
        {hasTemplates ? (
          <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'types' | 'templates')}>
      ...
      {activeTab === 'types' ? (
        ...
        <TemplatePalette
          ...
          selectedTag={selectedTemplateTag}
          onTagFilter={setSelectedTemplateTag}
        />
      )}
    </div>
  )
